...
async def select_tag(ctx, bot, subject):
  class AskView(View):
      def __init__(self):
      ...
      async def yes_callback(self, interaction, button):
        if interaction.user == ctx.author:
          self.clear_items()
          await interaction.response.edit_message(content='Updating tag', view = self)
          self.value = 1
          self.stop()
      ...
      async def no_callback(self, interaction, button):
        if interaction.user == ctx.author:
          self.clear_items()
          await interaction.response.edit_message(content='Not updating tag', view = self)
          self.value = 0
          self.stop()

  view = AskView()
  msg = await ctx.send('Do you want to update the tag?', view = view)
  def check(m):
  ...
  if tasks[1] in done_tasks:
    ...
    rq.update_tag(str(view.value), subject)
    return True